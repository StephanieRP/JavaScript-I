...
inventory.forEach(function(carID){
    if(carID.id == 33) {
...
// console.log(`The last car in your inventory is ${lastCar.car_make}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];

inventory.forEach(function(model) {
   let newArray = model.car_model;
   carModels.sort();
   carModels.push(newArray);
})
...
inventory.forEach(function(old) {
    if(old.car_year < 2000) {
        oldCars.push(old)
    }
})
